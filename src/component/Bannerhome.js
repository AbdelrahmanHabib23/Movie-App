          ...
          {bannerData.map((data, index) => (  
            <div key={data.id+"BannerHome"+index} className=" min-w-full min-h-[450px] lg:min-h-full  relative group transition all " style = {{transform : `translateX(-${currentImage * 100}%`}}>  
              ...
              <div className="container mx-auto ml-10 absolute bottom-0 max-w-md">  
                <div className="w-full absolute bottom-0 max-w-md px-14">  
                  ...
                  <Link  to= {"/"+data?.media_type+'/'+data.id}className="bg-black px-4 py-2 h-full text-white font-bold rounded mt-4 hover:bg-gradient-to-l from-blue-700 to-orange-500 shadow-md transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">  
                    Play Now  
                  </Link>  
                </div>  
              </div>  
